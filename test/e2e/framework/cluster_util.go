...
func (f *Framework) InitAClusterForTesting() string {
	// Be careful, mysql allowed hostname lenght is <63.
	cluster := newCluster(fmt.Sprintf("cl-%d", rand.Int31()/1000), f.Namespace.Name)
...
func (f *Framework) WaitClusterReadiness(cluster *apiv1alpha1.MysqlCluster) time.Duration {
	startTime := time.Now()
	timeout := f.Timeout
